function BlackjackBasicOmega(numDecks, numHands, numGames)
...
%                  clc;
    ...
    plot(results)
    title('Basic Strategy with Omega Card Counting - 10000, 100 Game Sessions')
...
function updateCount(card)
    ...
    %OMEAGA COUNT
    player.runningCount = player.runningCount + card.OmegaValue;
    %Counting the aces
    if(card.BJvalue == 11)
...
function betAmount = determineBetAmount
    ...
    decksUsed = round(deckPosition/52);
    ...
    %Condition will be met when the aces are at a greater proportion to
    %normal
    if(player.aceCount < 4*decksUsed)
