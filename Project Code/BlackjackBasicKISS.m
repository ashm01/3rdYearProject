function BlackjackBasicKISS(numDecks, numHands, numGames)
...
%                  clc;
    ...
    plot(results)
    title('Basic Strategy with KISS Card Counting - 10000, 100 Game Sessions')
...
function updateCount(card)
    ...
    %KISS COUNT
    %With KISS SYSTEM WE START WITH A COUNT OF ( AND INCREASE BET SIZE WHEN
    %THE RUNNING COUNT IS ABOVE 20
    ...
    %by the number of decks left this is not used for the kiss count
...
function betAmount = determineBetAmount
    ...
    if(player.runningCount > 19 && player.runningCount <= 22 )
        betAmount = 2;
    elseif(player.runningCount > 22 && player.runningCount <= 24)
        betAmount = 4;
    elseif(player.runningCount > 24 && player.runningCount <= 26)
        betAmount = 8;
    elseif(player.runningCount > 27 )
        betAmount = 15;
    else
        betAmount = 1;
    end
