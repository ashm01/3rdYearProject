function ShuffleCards(NumDecks)
...
global CARDS
CARDS.Rank = randperm(52*NumDecks);
CARDS.CardCount(1:13)=0;
...
for n = 1:(52*NumDecks)
    ...
    elseif (CARDS.Rank(n) <= 2*NumDecks*4)
        CARDS.Rank(n) = 2;
        CARDS.Value(n) = 2;
        CARDS.CardCount(2) = CARDS.CardCount(2) + 1;
    elseif (CARDS.Rank(n) <= 3*NumDecks*4)
        CARDS.Rank(n) = 3;
        CARDS.Value(n) = 3;
        CARDS.CardCount(3) = CARDS.CardCount(3) + 1;
    elseif (CARDS.Rank(n) <= 4*NumDecks*4)
        CARDS.Rank(n) = 4;
        CARDS.Value(n) = 4;
